...
/**
 * 「昨日」(Asia/Tokyo) の YYYY-MM-DD。
 * 相対日付（実行日の前日）+ JST 壁時計 21:00 で睡眠をシードすることで、
 * どの時刻に実行しても「前夜開始・未終了の睡眠が翌日の logs 窓に現れない」
 * 日跨ぎ条件が決定的に成立する（Date.UTC ベースで TZ 非依存に前日を計算）。
 */
...
test("日跨ぎアクティブ睡眠 (B-01): 前夜開始・未終了の睡眠を翌日 /baby のトグルから終了できる", async ({
  page,
  approvedUser,
}) => {
  // ── 1. login → 世帯作成（golden path と同じ導線）─────────────────────
  await loginViaMagicLink(page, approvedUser.email)
  await expect(page).toHaveURL(/\/setup/, { timeout: 15_000 })
  await page.getByLabel("世帯名").fill("E2E日跨ぎ睡眠世帯")
  await page.getByRole("button", { name: "世帯を作成する" }).click()
  await expect(page).toHaveURL(/\/meals/, { timeout: 15_000 })

  const householdId = await waitForHouseholdId(approvedUser.id)

  // ── 2. service role で「前夜 21:00 JST 開始・未終了」の睡眠をシード ──
  const admin = adminClient()
  const sleepStartIso = `${yesterdayJst()}T21:00:00+09:00`
  const { data: seeded, error: seedError } = await admin
    ...
    .insert({
      ...
      logged_at: sleepStartIso,
    })
    .select("id")
    .single()
  if (seedError || !seeded) {
    throw new Error(`sleep seed failed: ${seedError?.message}`)
  }

  // ── 3. /baby: トグルが経過時間付きの睡眠中表示（「ねんね」ではない）──
  await page.getByRole("link", { name: "育児" }).click()
  await expect(page).toHaveURL(/\/baby/, { timeout: 15_000 })
  await expect(page.getByText("睡眠中")).toBeVisible({ timeout: 15_000 })
  // 経過は最短でも 3 時間超（21:00 → 翌 0:00+）ゆえ /時間/ が必ず含まれる
  const wakeToggle = page.getByRole("button", { name: /時間/ })
  await expect(wakeToggle).toBeVisible()
  await expect(
    page.getByRole("button", { name: "ねんね" }),
  ).not.toBeVisible()

  // ── 4. タップ 1 回で終了 → 終了トースト（23505 袋小路が出ないこと）──
  await wakeToggle.click()
  await expect(page.getByText(/おはよう！/)).toBeVisible({ timeout: 15_000 })

  // ── 5. フォールバック明示クリア: トグルが「ねんね」へ戻る ────────────
  await expect(page.getByRole("button", { name: "ねんね" })).toBeVisible()
  await expect(page.getByText("起きてる")).toBeVisible()

  // ── 6. DB 断面: ended_at が設定されている（決定的な完了確認）──────────
  await expect(async () => {
    const { data: row, error } = await admin
      ...
      .eq("id", seeded.id)
      .single()
    if (error) throw new Error(`sleep lookup failed: ${error.message}`)
    if (!row.ended_at) throw new Error("ended_at not set yet")
  }).toPass({ timeout: 15_000 })

  // ── 7. teardown は fixtures/test.ts の approvedUser が世帯ごと削除 ──
})
